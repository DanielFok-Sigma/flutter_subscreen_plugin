...
class FlutterDualScreenPlugin : FlutterPlugin, ActivityAware, MethodCallHandler {
...
//    private lateinit var subChannel: MethodChannel
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        ...
        when (call.method) {
            Constant.METHOD_SUPPORT_DOUBLE_SCREEN -> {
                val isMultipleScreen = FlutterDualScreenProvider.instance.supportViceScreen()
                result.success(isMultipleScreen)
            }

            Constant.METHOD_CHECK_OVERLAY_PERMISSION -> {
                //校验是否具有 overlay 权限
                result.success(FlutterDualScreenProvider.instance.checkOverlayPermission())
            }
            ...
            Constant.METHOD_DOUBLE_SCREEN_SHOW -> {
                //显示副屏
                FlutterDualScreenProvider.instance.showSubDisplay()
            }

            Constant.METHOD_DOUBLE_SCREEN_CANCEL -> {
                //关闭副屏
                FlutterDualScreenProvider.instance.closeSubDisplay()
            }
    ...
    override fun onAttachedToActivity(binding: ActivityPluginBinding) {
        ...
        FlutterDualScreenProvider.instance.setFlutterSubCallback(object : IFlutterDualCallback {
            override fun onSubFlutterEngineCreated() {
                //副屏 engine 初始化后，将副屏事件进行分发
                FlutterDualScreenProvider.instance.flutterEngine?.let { engine ->
        ...
        FlutterDualScreenProvider.instance.doInit(binding.activity, autoShowSubScreenWhenInit)
    }
    ...
    override fun onDetachedFromActivity() {
        ...
        FlutterDualScreenProvider.instance.onDispose()
    }
}
