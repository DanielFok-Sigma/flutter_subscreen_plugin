package com.ezyorder.flutter_dualscreen_plugin
...
class FlutterDualScreenProvider private constructor() {

    private var mediaRouter: MediaRouter? = null
    var currentActivity: Activity? = null
    var flutterEngine: FlutterEngine? = null
    var presentation: FlutterDualScreenPresentation? = null

    private var iCallback: IFlutterDualCallback? = null;

    companion object {
        val instance: FlutterDualScreenProvider by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
            FlutterDualScreenProvider()
        }
    }

    fun setFlutterSubCallback(callback: IFlutterDualCallback) {
        iCallback = callback
    }
    ...
    fun doInit(activity: Activity, showSubScreen: Boolean) {
        ...
        mediaRouter =
            activity.applicationContext.getSystemService(Context.MEDIA_ROUTER_SERVICE) as MediaRouter
    ...
    private fun doInitEngine() {
        currentActivity?.let { activity ->
            ...
            FlutterDualScreenPlugin.tripPlugins?.let {
                if (it.isNullOrEmpty()) {
                    it.forEach { plugin ->
            ...
            //指定初始化路由
            flutterEngine?.navigationChannel?.setInitialRoute(FlutterDualScreenPlugin.subMainRouter)
            flutterEngine?.dartExecutor?.executeDartEntrypoint(
                DartExecutor.DartEntrypoint(
                    FlutterInjector.instance().flutterLoader().findAppBundlePath(),
                    FlutterDualScreenPlugin.mainRouter
                )
            )
    ...
    fun configSecondDisplay(context: Context) {
        ...
        if (display != null) {
            ...
            flutterEngine?.let { engine ->
                try {
                    presentation = FlutterDualScreenPresentation(context, display, engine)
